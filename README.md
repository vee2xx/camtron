# Camtron
Camtron is a simple cross platform library written in go to consume and process a stream from a webcam. It uses Electron and the MediaDevices Web API to access the webcam and allows a variety of consumers to listen for and process the stream, for example saving the video to a file or sending it on to one or more endpoints. It is supported on Linux, Windows 10 and Macos. Currently the only supported codec is VP9. More will be added shortly.
